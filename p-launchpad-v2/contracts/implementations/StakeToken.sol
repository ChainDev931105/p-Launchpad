 ...
 * @author Eric Nordelo
 */
contract StakeToken is ERC721, AccessControl, Pausable, Initializable {
    ...
    uint128 public unstakeFee; // value between 1 and FEE_PRECISION

    mapping(uint256 => uint256) private tokensRewards;
    mapping(uint256 => uint256) private tokensRoundIndex;
    mapping(uint256 => uint256) private appTokensStaked;
    ...
    function getAccruedReward(uint256 _tokenId) external view returns (uint256 accruedReward) {
        ...
        accruedReward = appTokensStaked[_tokenId];
        ...
        uint256 minUnlockingDateAndCurrentDate = block.timestamp < unlockingDate // solhint-disable-line
        ...
        accruedReward +=
            (((appTokensStaked[_tokenId] * _fundingRoundsData[_roundId - 1].stakeReward) / REWARD_PRECISION) *
                (minUnlockingDateAndCurrentDate - roundEndingDate)) /
     ...
     */
    function getRoundInfo(uint256 _roundNumber) public view returns (FundingStakeRoundsData memory) {
        require(_roundNumber > 0, "Rounds index starts with 1");
        require(_fundingRoundsData[_roundNumber - 1].capTokensToBeStaked > 0, "Unexistent round");
